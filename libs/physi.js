window.Physijs = (function () {
    ...
    convertWorldPositionToObject = function (position, object) {
        ...
        _temp_matrix4_1
            .identity()
            .makeRotationFromQuaternion(object.quaternion);
        ...
        return _temp_vector3_1
            .sub(_temp_vector3_2)
            .applyMatrix4(_temp_matrix4_1);
    };
    ...
    Physijs.PointConstraint = function (objecta, objectb, position) {
        ...
        this.positiona = convertWorldPositionToObject(
            position,
            objecta
        ).clone();
        ...
        if (objectb) {
            ...
            this.positionb = convertWorldPositionToObject(
                position,
                objectb
            ).clone();
        }
     ...
     */
    Physijs.HingeConstraint.prototype.setLimits = function (
        low,
        high,
        bias_factor,
        relaxation_factor
    ...
    Physijs.HingeConstraint.prototype.enableAngularMotor = function (
        velocity,
        acceleration
    ) {
    ...
    Physijs.SliderConstraint.prototype.setLimits = function (
        lin_lower,
        lin_upper,
        ang_lower,
        ang_upper
    ...
    Physijs.SliderConstraint.prototype.setRestitution = function (
        linear,
        angular
    ) {
    ...
    Physijs.SliderConstraint.prototype.enableLinearMotor = function (
        velocity,
        acceleration
    ) {
    ...
    Physijs.SliderConstraint.prototype.enableAngularMotor = function (
        velocity,
        acceleration
    ) {
    ...
    Physijs.ConeTwistConstraint.prototype.setMaxMotorImpulse = function (
        max_impulse
    ) {
    ...
    Physijs.ConeTwistConstraint.prototype.setMotorTarget = function (target) {
        if (target instanceof THREE.Vector3) {
            target = new THREE.Quaternion().setFromEuler(
                new THREE.Euler(target.x, target.y, target.z)
            );
    ...
    Physijs.DOFConstraint.prototype.configureAngularMotor = function (
        which,
        low_angle,
        high_angle,
        velocity,
        ...
        for (id1 in this._objects) {
            ...
            if (collisions[id1]) {
                ...
                for (j = 0; j < object._physijs.touches.length; j++) {
                    if (
                        collisions[id1].indexOf(object._physijs.touches[j]) ===
                        -1
                    ) {
        ...
        for (i = 0; i < object.children.length; i++) {
            if (object.children[i]._physijs) {
                ...
                _temp_vector3_1.setFromMatrixPosition(
                    object.children[i].matrixWorld
                );
                _quaternion_1.setFromRotationMatrix(
                    object.children[i].matrixWorld
                );
        ...
        if (object._physijs) {
            ...
            if (object instanceof Physijs.Vehicle) {
                ...
                if (object.material._physijs) {
                    if (
                        !this._materials_ref_counts.hasOwnProperty(
                            object.material._physijs.id
                        )
                    ) {
                        this.execute(
                            'registerMaterial',
                            object.material._physijs
                        );
                        object._physijs.materialId =
                            object.material._physijs.id;
                        this._materials_ref_counts[
                            object.material._physijs.id
                        ] = 1;
    ...
    Physijs.PlaneMesh = function (geometry, material, mass) {
        ...
        this._physijs.mass =
            typeof mass === 'undefined' ? width * height : mass;
    };
    ...
    Physijs.HeightfieldMesh = function (geometry, material, mass, xdiv, ydiv) {
        ...
        this._physijs.xpts =
            typeof xdiv === 'undefined'
                ? Math.sqrt(geometry.vertices.length)
                : xdiv + 1;
        this._physijs.ypts =
            typeof ydiv === 'undefined'
                ? Math.sqrt(geometry.vertices.length)
                : ydiv + 1;
        // note - this assumes our plane geometry is square, unless we pass in specific xdiv and ydiv
        this._physijs.absMaxHeight = Math.max(
        ...
        for (var i = 0; i < geometry.vertices.length; i++) {
            ...
            b = Math.round(
                i / this._physijs.xpts -
                    (i % this._physijs.xpts) / this._physijs.xpts
            );
            points[i] =
                geometry.vertices[
                    a + (this._physijs.ypts - b - 1) * this._physijs.ypts
                ].z;
    ...
    Physijs.BoxMesh = function (geometry, material, mass) {
        ...
        this._physijs.mass =
            typeof mass === 'undefined' ? width * height * depth : mass;
    };
    ...
    Physijs.Vehicle.prototype.addWheel = function (
        wheel_geometry,
        wheel_material,
        connection_point,
        wheel_direction,
    ...
    Physijs.VehicleTuning = function (
        ...
        this.suspension_stiffness =
            suspension_stiffness !== undefined ? suspension_stiffness : 5.88;
        this.suspension_compression =
            suspension_compression !== undefined
                ? suspension_compression
                : 0.83;
        this.suspension_damping =
            suspension_damping !== undefined ? suspension_damping : 0.88;
        this.max_suspension_travel =
            max_suspension_travel !== undefined ? max_suspension_travel : 500;
        this.friction_slip = friction_slip !== undefined ? friction_slip : 10.5;
        this.max_suspension_force =
            max_suspension_force !== undefined ? max_suspension_force : 6000;
    };
